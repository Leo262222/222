...
const QuestionBox = ({ advisor, user, onLoginRequest }: { advisor: Advisor, user: any, onLoginRequest: () => void }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // 2. (模拟) 触发邮件通知逻辑 
      // 真实环境需要云函数，这里先模拟成功
      console.log(`Email trigger: User ${user.email} asked Advisor ${advisor.id}: ${content}`);
      ...
      <div className="bg-white border border-purple-100 rounded-xl shadow-sm overflow-hidden relative">
        {!user ? (
          ...
          <div className="p-8 text-center bg-green-50 animate-fade-in">
            ...
            <p className="text-xs text-green-600 mt-1">顾问已收到通知，回复将发送至您的邮箱。</p>
          </div>
        ) : (
          // --- 状态C: 已登录 (输入框) ---
          <div className="p-1">
            ...
            <div className="bg-gray-50 px-4 py-2 flex justify-between items-center border-t border-gray-100">
              <span className="text-[10px] text-gray-400">系统将通过邮件通知顾问</span>
              <button 
                onClick={handleSend}
...
const LoginModal = ({ isOpen, onClose, onLoginSuccess }: any) => {
  ...
  const handleSendCode = async (e: React.FormEvent) => {
    ...
    else { setStep('code'); setMessage('✅ 验证码已发送！请查收邮件'); }
    setLoading(false);
  };
        ...
        {step === 'email' && (
          <>
            <button onClick={handleGoogleLogin} className="w-full flex items-center justify-center gap-3 bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 font-medium py-2.5 rounded-xl transition-all mb-4">
              <img src="https://www.svgrepo.com/show/475656/google-color.svg" className="w-5 h-5" alt="G" /> 使用 Google 一键登录
            </button>
...
// --- 用户菜单 (UserMenu) ---
const UserMenu = ({ user, onLogout }: any) => {
  const [isOpen, setIsOpen] = useState(false);
  const menuRef = useRef<HTMLDivElement>(null);
  useEffect(() => {
    const handleClickOutside = (event: any) => { if (menuRef.current && !menuRef.current.contains(event.target)) setIsOpen(false); };
  ...
  return (
    <div className="relative" ref={menuRef}>
      <button onClick={() => setIsOpen(!isOpen)} className="flex items-center gap-2 focus:outline-none hover:opacity-80 transition-opacity"><img src={avatarUrl} alt="User" className="w-9 h-9 rounded-full border-2 border-green-500 shadow-sm" /></button>
      {isOpen && (
        <div className="absolute right-0 mt-3 w-56 bg-white rounded-xl shadow-2xl py-2 z-50 border border-gray-100 animate-scale-in origin-top-right">
          ...
          <div className="py-1"><button onClick={onLogout} className="w-full text-left px-5 py-2.5 text-sm text-red-500 hover:bg-red-50 transition-colors flex items-center gap-2"><span className="text-lg">🚪</span> 退出登录</button></div>
        </div>
      )}
    </div>
  );
    ...
    <div className="min-h-screen bg-gray-50 font-sans text-gray-900 pb-20">
      ...
      <header className={`bg-[#1a202c] text-white px-4 shadow-lg sticky top-0 z-40 transition-all duration-300 ${isScrolled ? 'py-3' : 'py-6'}`}>
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          ...
          <div className="flex-shrink-0 ml-4">{user ? <UserMenu user={user} onLogout={handleLogout} /> : <button onClick={() => setIsLoginOpen(true)} className="bg-[#10B981] hover:bg-[#059669] text-white text-xs font-bold px-4 py-2 rounded-full shadow-lg shadow-green-900/20 transition-transform hover:scale-105 flex items-center gap-2"><span>🚀</span> 登录 / 注册</button>}</div>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-4 mt-4 sticky top-[70px] z-30"> 
        <div className="bg-white p-2 rounded-xl shadow-sm border border-gray-100 flex gap-2 overflow-x-auto no-scrollbar">
          {categories.map(cat => (<button key={cat.id} onClick={() => setSelectedCategory(cat.value)} className={`px-4 py-2 rounded-lg text-sm font-bold whitespace-nowrap transition-colors ${selectedCategory === cat.value ? 'bg-purple-900 text-white shadow-md' : 'bg-gray-50 text-gray-500 hover:bg-gray-100'}`}>{cat.label.includes('(') ? cat.label.split('(')[0] : cat.label}</button>))}
        </div>
      </div>
        ...
        {loading ? (<div className="text-center py-20 text-gray-400"><p className="animate-pulse">✨ 正在连接宇宙能量...</p></div>) : (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8">
            {filteredAdvisors.map(advisor => {
              ...
              return (
                <div key={advisor.id} onClick={() => setSelectedAdvisor(advisor)} className="group bg-white rounded-2xl p-4 md:p-6 shadow-sm border border-gray-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer flex flex-row md:flex-col items-start md:items-center md:text-center gap-4 md:gap-6 relative overflow-hidden">
                  <div className="relative shrink-0"><img src={advisor.imageUrl} alt="Avatar" className="w-16 h-16 md:w-32 md:h-32 rounded-full object-cover border-2 border-white shadow-md bg-gray-100 group-hover:scale-105 transition-transform duration-500" loading="lazy" />{advisor.isOnline && <div className="hidden md:block absolute bottom-2 right-2 w-4 h-4 bg-green-500 border-2 border-white rounded-full animate-pulse"></div>}</div>
                  <div className="flex-1 min-w-0 w-full flex flex-col md:items-center">
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-3 md:justify-center">{safeTags.slice(0, 3).map((tag, i) => <span key={i} className="text-[10px] md:text-xs bg-purple-50 text-purple-700 px-2 py-1 rounded-full border border-purple-100">{tag}</span>)}</div>
                    <div className="flex md:flex-col justify-between items-center w-full border-t md:border-t-0 border-gray-50 pt-3 md:pt-0 mt-auto"><div className="md:mb-4"><span className="text-sm md:text-3xl font-bold text-gray-900">$ {advisor.pricePerMinute}</span><span className="text-xs md:text-sm text-gray-400"> / 分</span></div><div className="hidden md:block w-full"><button className="w-full bg-[#10B981] hover:bg-[#059669] text-white font-bold py-3 rounded-xl shadow-lg shadow-green-100 transition-colors flex items-center justify-center gap-2"><span className="text-xl">📞</span> 立即连线</button></div></div>
                  </div>
                </div>
              );
            })}
      ...
      {selectedAdvisor && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 animate-fade-in">
          ...
          <div className="relative bg-white w-full max-w-lg rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto animate-slide-up">
            <div className="sticky top-0 bg-white/95 backdrop-blur z-10 border-b px-6 py-4 flex justify-between items-center"><h3 className="font-bold text-lg">顾问详情</h3><button onClick={() => setSelectedAdvisor(null)} className="w-8 h-8 rounded-full bg-gray-100 text-gray-500 flex items-center justify-center hover:bg-gray-200">✕</button></div>
            <div className="p-6 space-y-6">
               <div className="text-center">
                ...
                <div className="flex justify-center gap-6 mt-6">
                   <div className="text-center"><div className="text-xl font-bold text-gray-900">${selectedAdvisor.pricePerMinute}</div><div className="text-xs text-gray-400">每分钟</div></div><div className="w-px bg-gray-200 h-10"></div>
                   <div className="text-center"><div className="text-xl font-bold text-gray-900">{selectedAdvisor.yearsExperience}年</div><div className="text-xs text-gray-400">经验</div></div><div className="w-px bg-gray-200 h-10"></div>
               ...
               <div className="bg-purple-50 rounded-xl p-6 border border-purple-100 text-center">{selectedAdvisor.bookingQrUrl ? (<><img src={selectedAdvisor.bookingQrUrl} className="w-32 h-32 mx-auto mix-blend-multiply mb-2"/><p className="text-xs text-purple-500">长按识别二维码，添加顾问微信</p></>) : <p className="text-gray-400 text-sm">暂无联系方式</p>}</div>
            </div>
          </div>
        </div>
      )}
