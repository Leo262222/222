    ...
    <div className="min-h-screen bg-gray-50 font-sans text-gray-900 pb-20">
      ...
      <div className="max-w-6xl mx-auto px-4 mt-4 sticky top-[70px] z-30"> 
        <div className="bg-white p-2 rounded-xl shadow-sm border border-gray-100 flex gap-2 overflow-x-auto no-scrollbar">
          {categories.map(cat => (
            <button key={cat.id} onClick={() => setSelectedCategory(cat.value)} className={`px-4 py-2 rounded-lg text-sm font-bold whitespace-nowrap transition-colors ${selectedCategory === cat.value ? 'bg-purple-900 text-white shadow-md' : 'bg-gray-50 text-gray-500 hover:bg-gray-100'}`}>
                ...
                <div key={advisor.id} onClick={() => setSelectedAdvisor(advisor)} className="group bg-white rounded-2xl p-4 md:p-6 shadow-sm border border-gray-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer flex flex-row md:flex-col items-start md:items-center md:text-center gap-4 md:gap-6 relative overflow-hidden">
                  ...
                  <div className="flex-1 min-w-0 w-full flex flex-col md:items-center">
                    <div className="flex md:flex-col justify-between md:justify-center items-start md:items-center w-full mb-1 md:mb-3">
                      ...
                      <div className="flex items-center text-yellow-500 text-xs md:text-sm font-bold bg-yellow-50 px-2 py-0.5 rounded md:mt-2"><span>★ {advisor.rating}</span><span className="hidden md:inline text-gray-400 font-normal ml-1">({advisor.yearsExperience}年)</span></div>
                    </div>
                    ...
                    <div className="flex md:flex-col justify-between items-center w-full border-t md:border-t-0 border-gray-50 pt-3 md:pt-0 mt-auto">
                      ...
                      <div className="hidden md:block w-full"><button className="w-full bg-[#10B981] hover:bg-[#059669] text-white font-bold py-3 rounded-xl shadow-lg shadow-green-100 transition-colors flex items-center justify-center gap-2"><span className="text-xl">📞</span> 立即连线</button></div>
                    </div>
                  </div>
                </div>
              );
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 animate-fade-in">
          ...
          <div className="relative bg-white w-full max-w-lg rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto animate-slide-up">
            ...
            <div className="p-6 space-y-8">
               ...
               {(selectedAdvisor.certificates || []).length > 0 && (<div><h4 className="text-sm font-bold text-gray-900 mb-3">资质认证</h4><div className="flex gap-3 overflow-x-auto pb-2">{selectedAdvisor.certificates?.map((cert, idx) => (<img key={idx} src={cert} onClick={() => setSelectedCertificate(cert)} className="h-20 rounded-lg border cursor-zoom-in" />))}</div></div>)}
